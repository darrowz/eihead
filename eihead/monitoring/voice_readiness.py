...
def build_voice_chain_readiness(payload: object | None = None) -> dict[str, object]:
    """Normalize voice-chain data into a monitor-facing readiness payload."""
    ...
    if isinstance(payload, Mapping):
        explicit = payload.get("explicit") if isinstance(payload.get("explicit"), Mapping) else None
        benchmark = payload.get("benchmark") if isinstance(payload.get("benchmark"), Mapping) else None
        scenario_targets = payload.get("scenario_targets") if isinstance(payload.get("scenario_targets"), Mapping) else None

        if explicit is None and "schema" in payload:
            explicit = payload
        if benchmark is None and "metrics" in payload:
            benchmark = payload

    if isinstance(explicit, Mapping):
        readiness = _readiness_from_explicit(explicit)
        return readiness
...
def _readiness_from_explicit(explicit: Mapping[str, Any]) -> dict[str, object]:
...
def _readiness_from_benchmark(benchmark: Mapping[str, Any]) -> dict[str, object]:
    ...
    if "roundLeakRate" in data:
        readiness["roundLeakRate"] = data["roundLeakRate"]
    if isinstance(data.get("thresholds"), Mapping):
        readiness["thresholds"] = _json_mapping(data["thresholds"])
    if isinstance(data.get("stageLatencyMetrics"), Mapping):
        readiness["stageLatencyMetrics"] = _json_mapping(data["stageLatencyMetrics"])
    if isinstance(streaming, Mapping) and streaming:
        readiness["streaming"] = _json_mapping(streaming)
    if isinstance(interrupt_stop, Mapping) and interrupt_stop:
        readiness["interruptStop"] = _json_mapping(interrupt_stop)
    if isinstance(readiness_summary, Mapping) and readiness_summary:
        readiness["readinessSummary"] = _json_mapping(readiness_summary)
    readiness["summary"] = _summary(readiness)
    readiness["readinessMessage"] = _readiness_message(readiness)
    return readiness
